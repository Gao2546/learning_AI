...
// 1. GenerateImage
export type ResultGenerateModel = {
  result: string;
  data_path?: string;
};
...
async function generateModel(prompt: string, imgUrl: string) {
  try {
...
async function clickElement(id: string, className: string, tagName: string) {
  try {
