...
async function fetchTextFromWebsite(url: string): Promise<string> {
  try {
...
// let setting_data = await fetchTextFromWebsite('https://www.thairath.co.th/news/politic/2851132'); // Replace with your URL
...
async function addChatHistory(content : string | undefined | unknown | any | never): Promise<void> {
  ChatHistory.push(content);
}
await addChatHistory(setting_prompt);

async function llm(question: string): Promise<Record<string , any> | string | undefined> {
  ...
  if (responseText !== undefined) {
    const rrs: string = responseText; // Now it's safe to assign
...
async function main(): Promise<void> {
  while (true) {
      let input: string = await askQuestion("Enter something (type 'exit' to quit): ");
